...
void solve(void) {
  ...
  repi(k, 1, n+1) {
    repi(i, 1, n+1) {
      repi(j, 1, n+1) {
        // custom Warshal-Floyd here
        if(r[i][j].fi > r[i][k].fi + r[k][j].fi) {
          r[i][j] = r[i][k] + r[k][j];
        } else if(r[i][j].fi == r[i][k].fi + r[k][j].fi && r[i][j].se < r[i][k].se + r[k][j].se) {
          r[i][j] = r[i][k] + r[k][j];
        }
      }
    }
  ...
  rep(i, q) {
    ...
    if(r[u1][v1].se == -inf) {
      ...
      print2(r[u1][v1].fi, r[u1][v1].se + a[u1]);
    }
  }
}
