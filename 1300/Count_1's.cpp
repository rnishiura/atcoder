/*
Problem: https://atcoder.jp/contests/arc137/tasks/arc137_b
Author: rnishiura
Date: 23-01-21
Description:
*/
...
void solve() {
  ...
  ll max_dif = 0, min_dif = 0;
  repi(i, 1, n+1) {
    max_dif = max(max_dif, b[i]-min_val);
    min_dif = min(min_dif, b[i]-max_val);
    max_val = max(max_val, b[i]);
    min_val = min(min_val, b[i]);
  }
  // print(max_dif);
  // print(min_dif);
  // print2(max_val, min_val);
  print(max_dif-min_dif+1);
}
