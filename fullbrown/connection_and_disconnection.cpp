...
void solve() {
  ...
  set<char> t; rep(i, s.length()) t.insert(s[i]);
  ...
  vector<bool> r(s.length(), false);
  repi(i, 1, s.length()) {
    if(s[i-1] == s[i] && r[i-1] == false) r[i] = true;
  }
  vector<bool> u(s.length(), false);
  if(s[s.length()-1] == s[0] && r[s.length()-1] == false) u[0] = true;
  repi(i, 1, s.length()) {
    if(s[i-1] == s[i] && u[i-1] == false) u[i] = true;
  }
  ll cr = 0, cu = 0;
  for(auto e: r) if(e) cr++;
  for(auto e: u) if(e) cu++;
  print((k-1)*cu + cr);
}
