...
ll wins(ll i, ll j, ll r, auto& a) {
  bool ret = a[i][r] == 'G' && a[j][r] == 'C';
  ret |= a[i][r] == 'C' && a[j][r] == 'P';
  ret |= a[i][r] == 'P' && a[j][r] == 'G';
  return ret;
}
...
void solve() {
  ...
  rep(i, m) {
    ...
    rep(j, n) {
      auto g = *it++;
      auto h = *it++;
      if(wins(2*n-g.se, 2*n-h.se, i, a)) {
        pp.insert(mp(g.fi+1, g.se));
        pp.insert(h);
      } else if(wins(2*n-h.se, 2*n-g.se, i, a)) {
        pp.insert(mp(h.fi+1, h.se));
        pp.insert(g);
      } else {
        pp.insert(h);
        pp.insert(g);
