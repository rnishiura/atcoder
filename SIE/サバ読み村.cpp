...
void dfs(ll pos) {
  for(auto e: g[pos]) {
    if(hist[e.POS] == 0) {
      hist[e.POS] = hist[pos]+e.COST;
      same.insert(e.POS);
    }
    else if(hist[e.POS] != 0) {
      if(hist[e.POS] != hist[pos]+e.COST) {
        // print2(prev, pos);
        impossible = true;
        return;
      }
...
void solve() {
  ...
  repi(i, 1, n+1) {
    ...
    same.insert(i);
    dfs(i);
    ll max_v = -inf, min_v = inf;
    for(auto e: same) {
      max_v = max(hist[e], max_v);
      min_v = min(hist[e], min_v);
    } 
    ans = max(max_v-min_v, ans);
  }
  if(ans > 100) ans = -1;
  // print(hist);
  print(impossible?-1:ans);
}
