...
void solve() {
  ll n; cin >> n;
  ll XA=0, BX=0, BXA=0, AB=0;
  string s;
  rep(i, n) {
    cin >> s;
    rep(i, s.length()-1) { if(s[i] == 'A' && s[i+1] == 'B') AB++; }
    if(s[0] == 'B' && s[s.length()-1] == 'A') BXA++;
    else if(s[0]            == 'B') BX++;
    else if(s[s.length()-1] == 'A') XA++;
  }
  ll r;
  // ll r = AB + max(0LL, BXA-1) + min(XA, BX) + min(min(BXA, abs(XA-BX)), 1LL);
  if(BXA == 0) {
    r = AB + min(XA, BX);
  } else if(XA + BX == 0) {
    r = AB + BXA - 1;
