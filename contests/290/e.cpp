...
void solve() {
  ...
  set<ll> key; map<ll, v> val;
  ll i=0;
  for(auto e: a){
    key.insert(e);
    val[e].pb(min(i+1, n-i));
    i++;
  };
  ...
  rep(i, n/2) {
    if((n-2*i-1) % 2 == 0)
      ans += (n-2*i-1)*(n-2*i-1)/2+(n-2*i-1)/2;
    else 
      ans += (n-2*i-1)*(n-2*i-1)/2+(n-2*i-1)/2+1;
  }
  print(ans-r);
}
