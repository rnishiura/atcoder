/*
Problem: https://atcoder.jp/contests/abc244/tasks/abc244_e
Author: rnishiura
Date: 23-
Description:
- odd / even times
...
void solve() {
  ...
  vector<vv> dp(k+1, vv(n+1, v(2, 0)));
  dp[0][s][0] = 1;
  repi(i, 1, k+1) {
    repi(j, 1, n+1) {
      for(ll fr: g[j]) {
        if(fr == x) {
          dp[i][fr][0] += dp[i-1][j][1];
          dp[i][fr][1] += dp[i-1][j][0];
        } else {
          dp[i][fr][1] += dp[i-1][j][1];
          dp[i][fr][0] += dp[i-1][j][0];
        }
        dp[i][fr][0] %= MOD;
        dp[i][fr][1] %= MOD;
      }
